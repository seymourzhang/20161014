...
.controller("homeCtrl",function($scope, $state, $http, $ionicPopup,$ionicLoading,$cordovaFileTransfer,$cordovaFileOpener2,$timeout, AppData) {
		   		...
		   		if (data.ResponseDetail.Result == "Success") {
		   			if (data.ResponseDetail.NeedAlarm == 'Y') {
		   				...
		   				var myPopup = $ionicPopup.show({
							  title: '截止到昨天'+ showList +'栋，尚有生产记录未填写。',
							 scope: $scope,
							 buttons: [
							   { text: '稍后填' },
							   {
	...
	$scope.goProdReco = function(){
		Sparraw.getInfoStatus($ionicPopup,$state,"prodReco");
	}
...
.controller("prodPerfStanTableCtrl",function($scope, $state, $http, AppData) {
	...
	$scope.listArray = ['科宝(2015)','罗斯(2014)','AA+(2014)','正大笼养','正大平养','我的标准'];
...
.controller("setStandardCtrl",function($scope, $state,  $stateParams, $http, AppData) {
	...
	$scope.judgeStandard = function(){
		/*if ($scope.setStandardData.judgeFirstTime) {
			app_confirm("改变标准后数据将被覆盖，请确认是否修改！",'提示',null,function(buttonIndex){
	           	if(buttonIndex == 2){
	              	$scope.queryStandard();
	           	}else{
	           		$scope.setStandardData.SettingData.farm_standard = $scope.setStandardData.initial;
	           		$scope.queryStandard();
	           	}
		    });
		}else{
	...
	$scope.save = function(){
		...
		$scope.setStandardData.SettingData.cum_mortality.week1 = NulltoZero($scope.setStandardData.SettingData.cum_mortality.week1);
		$scope.setStandardData.SettingData.cum_mortality.week2 = NulltoZero($scope.setStandardData.SettingData.cum_mortality.week2);
		$scope.setStandardData.SettingData.cum_mortality.week3 = NulltoZero($scope.setStandardData.SettingData.cum_mortality.week3);
		$scope.setStandardData.SettingData.cum_mortality.week4 = NulltoZero($scope.setStandardData.SettingData.cum_mortality.week4);
		$scope.setStandardData.SettingData.cum_mortality.week5 = NulltoZero($scope.setStandardData.SettingData.cum_mortality.week5);
		$scope.setStandardData.SettingData.cum_mortality.week6 = NulltoZero($scope.setStandardData.SettingData.cum_mortality.week6);
		$scope.setStandardData.SettingData.cum_mortality.week7 = NulltoZero($scope.setStandardData.SettingData.cum_mortality.week7);
		$scope.setStandardData.SettingData.manage_alert.Max_Mortality = NulltoZero($scope.setStandardData.SettingData.manage_alert.Max_Mortality);
		$scope.setStandardData.SettingData.manage_alert.Max_feed = NulltoZero($scope.setStandardData.SettingData.manage_alert.Max_feed);
		$scope.setStandardData.SettingData.manage_alert.Min_body_weight = NulltoZero($scope.setStandardData.SettingData.manage_alert.Min_body_weight);


		var TempProdKeyArr = GetKey(myConfig.prodStan);
		var TempProdValueArr = GetValue(myConfig.prodStan);
...
.controller("dailyCumuCtrl",function($scope, $state, $http,$ionicModal, $stateParams, $ionicPopup, AppData) {
	...
	$scope.goDailyDay = function(){
		selectBackPage.cumuGoDay = "dailyCumu";
